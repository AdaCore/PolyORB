--  The package Namet is a part of the GNAT compiler.
...
package body CIAO.Namet is
   ...
   package Name_Chars is new GNAT.Table (
     ...
     Table_Low_Bound      => 0,
     Table_Initial        => CIAO.Alloc.Name_Chars_Initial,
     Table_Increment      => CIAO.Alloc.Name_Chars_Increment);
   ...
   package Name_Entries is new GNAT.Table (
     ...
     Table_Initial        => CIAO.Alloc.Names_Initial,
     Table_Increment      => CIAO.Alloc.Names_Increment);
...
-- XXX |      F : array (Int range 0 .. Max_Chain_Length) of Int;
   ...
   begin
      null; --  Entire procedure body commented out.
-- XXX |      if Debug_Flag_H then
-- XXX |
-- XXX |         for J in F'Range loop
-- XXX |            F (J) := 0;
-- XXX |         end loop;
-- XXX |
-- XXX |         for I in Hash_Index_Type loop
-- XXX |            if Hash_Table (I) = No_Name then
-- XXX |               F (0) := F (0) + 1;
-- XXX |
-- XXX |            else
-- XXX |               Write_Str ("Hash_Table (");
-- XXX |               Write_Int (Int (I));
-- XXX |               Write_Str (") has ");
-- XXX |
-- XXX |               declare
...
-- XXX |                  N := Hash_Table (I);
-- XXX |
-- XXX |                  while N /= No_Name loop
-- XXX |                     N := Name_Entries.Table (N).Hash_Link;
-- XXX |                     C := C + 1;
-- XXX |                  end loop;
-- XXX |
-- XXX |                  Write_Int (C);
-- XXX |                  Write_Str (" entries");
-- XXX |                  Write_Eol;
-- XXX |
-- XXX |                  if C < Max_Chain_Length then
-- XXX |                     F (C) := F (C) + 1;
-- XXX |                  else
-- XXX |                     F (Max_Chain_Length) := F (Max_Chain_Length) + 1;
-- XXX |                  end if;
-- XXX |
-- XXX |                  N := Hash_Table (I);
-- XXX |
-- XXX |                  while N /= No_Name loop
-- XXX |                     S := Name_Entries.Table (N).Name_Chars_Index;
-- XXX |                     Write_Str ("      ");
-- XXX |
-- XXX |                     for J in 1 .. Name_Entries.Table (N).Name_Len loop
-- XXX |                        Write_Char (Name_Chars.Table (S + Int (J)));
-- XXX |                     end loop;
-- XXX |
-- XXX |                     Write_Eol;
-- XXX |                     N := Name_Entries.Table (N).Hash_Link;
...
-- XXX |         Write_Eol;
-- XXX |
-- XXX |         for I in Int range 0 .. Max_Chain_Length loop
-- XXX |            if F (I) /= 0 then
-- XXX |               Write_Str ("Number of hash chains of length ");
-- XXX |
-- XXX |               if I < 10 then
-- XXX |                  Write_Char (' ');
-- XXX |               end if;
...
-- XXX |               Write_Int (I);
-- XXX |
-- XXX |               if I = Max_Chain_Length then
-- XXX |                  Write_Str (" or greater");
-- XXX |               end if;
-- XXX |
-- XXX |               Write_Str (" = ");
-- XXX |               Write_Int (F (I));
...
-- XXX |               if I /= 0 then
-- XXX |                  Nsyms := Nsyms + F (I);
-- XXX |                  Probes := Probes + F (I) * (1 + I) * 100;
-- XXX |               end if;
-- XXX |            end if;
-- XXX |         end loop;
-- XXX |
-- XXX |         Write_Eol;
-- XXX |         Write_Str ("Average number of probes for lookup = ");
-- XXX |         Probes := Probes / Nsyms;
-- XXX |         Write_Int (Probes / 200);
-- XXX |         Write_Char ('.');
-- XXX |         Probes := (Probes mod 200) / 2;
-- XXX |         Write_Char (Character'Val (48 + Probes / 10));
-- XXX |         Write_Char (Character'Val (48 + Probes mod 10));
-- XXX |         Write_Eol;
-- XXX |         Write_Eol;
-- XXX |      end if;
   end Finalize;
   ...
   begin
      ...
      if In_Character_Range (C) then
         ...
         --  Improper Char_Code (Wide_Chars not supported in CIAO).
...
-- XXX |   procedure wn (Id : Name_Id) is
-- XXX |   begin
-- XXX |      Write_Name (Id);
-- XXX |      Write_Eol;
-- XXX |   end wn;
...
-- XXX |   procedure Write_Name (Id : Name_Id) is
-- XXX |   begin
-- XXX |      if Id >= First_Name_Id then
-- XXX |         Get_Name_String (Id);
-- XXX |         Write_Str (Name_Buffer (1 .. Name_Len));
-- XXX |      end if;
-- XXX |   end Write_Name;

-- XXX |   ------------------------
-- XXX |   -- Write_Name_Decoded --
-- XXX |   ------------------------

-- XXX |   procedure Write_Name_Decoded (Id : Name_Id) is
-- XXX |   begin
-- XXX |      if Id >= First_Name_Id then
-- XXX |         Get_Decoded_Name_String (Id);
-- XXX |         Write_Str (Name_Buffer (1 .. Name_Len));
-- XXX |      end if;
-- XXX |   end Write_Name_Decoded;
