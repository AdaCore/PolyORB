...
with CIAO.Hostparm; use CIAO.Hostparm;
with CIAO.Types;    use CIAO.Types;
with System;        use System;

package CIAO.Namet is
...
-- XXX |   procedure Write_Name (Id : Name_Id);
-- XXX |   --  Write_Name writes the characters of the specified name using the
-- XXX |   --  standard output procedures in package Output. No end of line is
-- XXX |   --  written, just the characters of the name. On return Name_Buffer and
-- XXX |   --  Name_Len are set as for a call to Get_Name_String. The name is written
-- XXX |   --  in encoded form (i.e. including Uhh, Whhh, Qx, _op as they appear in
-- XXX |   --  the name table). If Id is Error_Name, or No_Name, no text is output.
-- XXX |
-- XXX |   procedure wn (Id : Name_Id);
-- XXX |   --  Like Write_Name, but includes new line at end. Intended for use
-- XXX |   --  from the debugger only.
-- XXX |
-- XXX |   procedure Write_Name_Decoded (Id : Name_Id);
-- XXX |   --  Like Write_Name, except that the name written is the decoded name, as
-- XXX |   --  described for Name_Decode, and the resulting value stored in Name_Len
-- XXX |   --  and Name_Buffer is the decoded name.
-- XXX |
end CIAO.Namet;
