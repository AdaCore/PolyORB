...
--  a correct assumption, a significant number of changes will be required!
   ...
   --  In CIAO, the universal arithmetic facilities of GNAT is not
   --  used; Uint values are simply used as biased integer values.
   --  Ureal and Char_Codes are not used at all.
   ...
   Uint_Bias       : constant := (Uint_Low_Bound + Uint_High_Bound) / 2;
   --  In CIAO, this is the bias for storage of integers as
   --  Uints.
   ...
   --  Biased integer type --
   --------------------------
   type Unbiased_Uint is new Integer;
   type Uint is range Uint_Low_Bound .. Uint_High_Bound;

   function To_Uint   (N : Unbiased_Uint)
     return Uint;
   pragma Inline (To_Uint);

   function From_Uint (N : Uint)
     return Unbiased_Uint;
   pragma Inline (From_Uint);
