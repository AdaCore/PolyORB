...
-- XXX |   function New_List_Copy (List : List_Id) return List_Id;
-- XXX |   --  Creates a new list containing copies (made with Atree.New_Copy) of every
-- XXX |   --  node in the original list. If the argument is No_List, then the returned
-- XXX |   --  result is No_List. If the argument is an empty list, then the returned
-- XXX |   --  result is a new empty list.
-- XXX |
-- XXX |   function New_List_Copy_Original (List : List_Id) return List_Id;
-- XXX |   --  Same as New_List_Copy but copies only nodes coming from source
-- XXX |
-- XXX |   function New_List_Copy_Tree (List : List_Id) return List_Id;
-- XXX |   --  Similar to New_List_Copy, except that the copies are done using the
-- XXX |   --  Atree.New_Copy_Tree function, which means that a full recursive copy
-- XXX |   --  of the subtrees in the list is performed, setting proper parents. As
-- XXX |   --  for New_Copy_Tree, it is illegal to attempt to copy extended nodes
-- XXX |   --  (entities) either directly or indirectly using this function.
