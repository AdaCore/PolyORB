...
package body AdaBroker.OmniObject is

   use type System.Address;
   use type CORBA.String;
   use type CORBA.Unsigned_Long;

   Flag : constant Natural := AdaBroker.Debug.Is_Active ("omniobject");
   ...
   begin
      pragma Debug (O ("Omniobject.Init_Local_Object"));
   ...
   begin
      ...
      if Result /= null then
         ...
         pragma Debug (O ("Omniobject.Create_Omniobject : setting" &
   ...
   is
      ...
      Result   : Object_Ptr;
   begin
      pragma Debug (O ("Omniobject_Duplicate : entering ..."));

      C_Arg := Address_To_Object.To_Address (From_Object_Ptr (Self));

      pragma Debug (O ("Omniobject_Duplicate : got the C++ pointer," &
                       " calling the C++ function"));

      C_Result := C_Omniobject_Duplicate (C_Arg);

      pragma Debug (O ("OmniObject_Duplicate : got C++ function result"));

      Result := To_Object_Ptr (Address_To_Object.To_Pointer (C_Result));

      pragma Debug (O ("OmniObject_Duplicate : got the Ada pointer "));

      if Result /= null then
         Result.all.Implobj := null;
      end if;

      pragma Debug (O ("OmniObject_Duplicate : exiting ... OK"));
      return Result;
   ...
   begin
      pragma Debug (O ("Omniobject_Destructor : enter"));

      C_Omniobject_Destructor
        (Address_To_Object.To_Address (From_Object_Ptr (Self)));

      pragma Debug (O ("Omniobject_Destructor : leave"));
   ...
   function C_Non_Existent (Self : in Object'Class) return Sysdep.Bool;
   ...
   begin
      pragma Debug (O ("Omniobject.Initialize"));
   ...
   begin
      ...
      if not Is_Nil (Self) then
         pragma Debug (O ("Finalizing non nil object"));

         OmniObject_Destructor (Self.OmniObj);

         pragma Debug (O ("Finalize : object destroyed"));
   ...
   function C_Is_Proxy (Self : in Object'Class) return Sysdep.Bool;
   ...
   begin
      ...
      return Sysdep.To_Bool (Self.Implobj.all.Is_A (Rep));
   end C_Is_A;
