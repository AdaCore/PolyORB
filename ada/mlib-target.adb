...
package body MLib.Target is
   ...
   is
      DLL_File : constant String :=
        "lib" & MLib.Files.Ext_To (Lib_Filename, DLL_Ext);
   ...
   begin
      if Verbose then
         if Relocatable then
            Text_IO.Put_Line
              ("building relocatable shared library " & DLL_File);
         else
            Text_IO.Put_Line
              ("building non-relocatable shared library..." & DLL_File);
         end if;
      end if;
