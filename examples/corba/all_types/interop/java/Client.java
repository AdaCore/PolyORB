...
public class Client
{
	...
	all_types ato = all_typesHelper.narrow (obj);
	...
	// Enum type.
	all_typesPackage.Color col = all_typesPackage.Color.Blue;
	System.out.println ("echoColor   " + (all_typesPackage.Color.Blue == ato.echoColor (col)));
	...
	// Simple struct.
	all_typesPackage.simple_struct simple_s 
	    = new all_typesPackage.simple_struct (123, "Hello World!\n");
	all_typesPackage.simple_struct res_simple_s = ato.echoStruct (simple_s);
	...
	// Nested struct.
	all_typesPackage.nested_struct nested_s
	    = new all_typesPackage.nested_struct (simple_s);
	all_typesPackage.nested_struct res_nested_s = ato.echoNestedStruct (nested_s);

	pass = (nested_s.ns.a == res_nested_s.ns.a) && (nested_s.ns.s.equals (res_nested_s.ns.s));
	System.out.println ("echoNestedStruct " + pass);

	// Object ref.
	all_types res_obj = ato.echoRef (ato);
	// pass = org.omg.CORBA.portable.Delegate.is_equivalent(res_obj, ato);
	// XXX how to test this ?
	System.out.println ("echoRef " + true);
	
	// Union.
	all_typesPackage.myUnion union_t = new all_typesPackage.myUnion(); 
	union_t.Counter (4331);

	all_typesPackage.myUnion res_union_t = ato.echoUnion (union_t);
	System.out.println ("echoUnion " + (res_union_t.Counter() == 4331));

	// Union switch.
	all_typesPackage.myUnionEnumSwitch union_es_t 
	    = new all_typesPackage.myUnionEnumSwitch(); 
	union_es_t.foo (4331);

	all_typesPackage.myUnionEnumSwitch res_union_es_t 
	...
	// Attributes.
	ato.myColor (all_typesPackage.Color.Blue);
	System.out.println ("Attributes " + (all_typesPackage.Color.Blue == ato.myColor()));
	...
	catch (all_typesPackage.my_exception ex)
	    {
