...
package body PolyORB.Binding_Data.GIOP.UIPMC is
   ...
   begin
      ...
      --  From here on we know that both Right is of type UIPMC_Profile_Type

      --  Compare UIPMC versions

      if not (Left.Version_Major = UIPMC_Profile_Type (Right).Version_Major
              and then
                Left.Version_Minor = UIPMC_Profile_Type (Right).Version_Minor)
      then
         return False;
      end if;
