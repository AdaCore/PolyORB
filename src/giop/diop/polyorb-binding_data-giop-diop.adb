...
package body PolyORB.Binding_Data.GIOP.DIOP is
   ...
   begin
      ...
      --  From here on we know that Right is of type DIOP_Profile_Type

      --  Compare DIOP versions

      if not (Left.Version_Major = DIOP_Profile_Type (Right).Version_Major
              and then
              Left.Version_Minor = DIOP_Profile_Type (Right).Version_Minor)
      then
         return False;
      end if;
      ...
      begin
         ...
         R_Mechanisms := DIOP_Profile_Type (Right).Mechanisms;
