...
package body PortableServer.IdAssignmentPolicy is
   ...
   function Create_IdAssignmentPolicy
   ...
   is
   begin
      pragma Assert (The_Type = ID_ASSIGNMENT_POLICY_ID);

      if Get_Type (Value) /= TC_IdAssignmentPolicyValue then
...
begin
   Register_Module
     (Module_Info'
      (Name      => +"portableserver.idassignmentpolicy",
       Conflicts => Empty,
