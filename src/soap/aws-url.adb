...
package body AWS.URL is
   ...
   begin
      ...
      if URL (URL'First .. URL'First + HTTP_Token'Length - 1)
        = HTTP_Token (HTTP_Token'Range)
      then
         O.Port := Default_HTTP_Port;
         Parse (URL (URL'First + HTTP_Token'Length .. URL'Last));

      elsif URL (URL'First .. URL'First + HTTPS_Token'Length - 1)
        = HTTPS_Token (HTTPS_Token'Range)
      then
