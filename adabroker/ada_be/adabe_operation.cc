  ...
  // previous contains the local definition of complexe types.
{
  ...
  body += "      Operation : " + name_of_the_package + ".Proxy.";
  ...
  // Special code for oneway operation.
  if (oneway)
    {
      body += "One_Way (Self, Operation);\n";
    }
  else
    {
      // if there is out argument (s), we need a get_result function
      body += "Invoke (Self, Operation);\n";
      if (is_function ()) {
      ...
      else if (!no_out)
	{
	  ...
	  body += ".Proxy.Get_Result\n";
	  body += "         (Operation" + out_args;
	}
    }
...
////////////////////////////////////////////////////////////////////////
void
adabe_operation::produce_proxy_ads (dep_list & with,
				    string   & body,
				    string   & private_definition)
  // Produce the proxy_ads_file.
  // with is the dependence-list.
  // body is th main part of the file.
  // private_definition contains the private part of the mapping.
{
  ...
  // Fields contains the string of all arguments pointer. Added in the
  // private part.
  string fields = "";
  ...
  while (!i.is_done ())
    {
      AST_Decl *d = i.item ();
      if (d->node_type () == AST_Decl::NT_argument)
	// Produce the different argument strings. The cast is here
	// explicit in order to call the right method.
	dynamic_cast<adabe_argument *>(d)->produce_proxy_ads
	  (with, in_decls, no_in, no_out, fields, out_args);
      else throw adabe_internal_error
	     (__FILE__,__LINE__,"Unexpected node in operation");
      i.next ();
    }
  ...
  if ((!return_is_void ()) && (!is_function ())) 
    {
      //  Result is mapped in an out argument
      AST_Decl *b = return_type ();
      string tmp =
	dynamic_cast<adabe_name *>(b)->dump_name (with, private_definition);
      out_args += ";\n      Returns : out " + tmp;
      fields += "      Returns : ";
      fields += tmp;
      fields += ";\n";
...
////////////////////////////////////////////////////////////////////////
void
adabe_operation::produce_proxy_adb (dep_list & with,
				    string   & body,
				    string   & private_definition)
  // This method produce the proxy_adb_file,
  // with is the dependence-list.
  // body is th main part of the file.
  // private_definition contains the private part of the mapping.
{
  ...
  // see adabe_argument.cc in produce_proxy_adb for details 
  string result_name = "";
  string in_decls = "";
  string init = "";
  string align_size = "";
  ...
  while (!i.is_done ())
    {
      AST_Decl *d = i.item ();
      if (d->node_type () == AST_Decl::NT_argument)
	// Call the produce_proxy_adb of the argument. There is a
	// direct cast because this procedure is not virtual
	dynamic_cast<adabe_argument *>(d)->produce_proxy_adb
  ...
  if (user_exceptions) {
	...
	dynamic_cast<adabe_exception *>(d)->produce_proxy_adb (with, tmp);
	body += tmp;
    ...
    body += "        (CORBA.AdaBroker_Fatal_Error'Identity,\n";
    body += "         \"" + get_ada_local_name ();
    body += "_Proxy.User_Exception: unknown exception\");\n";
    body += "   end User_Exception;\n\n";
  }
  ...
  // private_definition contains the private part of the mapping.
{
  ...
  while (!i.is_done ())
	...
	dynamic_cast<adabe_argument *>(d)->produce_skel_adb
	  (with, in_decls, no_in, no_out,
	   unmarshall, call_args, marshall, align_size);
      else throw adabe_internal_error
	     (__FILE__,__LINE__,"Unexpected node in operation");
      i.next ();
    }
  ...
  body += "      if Orl_Op = \"" + get_ada_local_name () + "\" then\n";
  body += "         declare\n";
  body += in_decls;
  ...
  if (oneway) {
    body += "            -- Check the client does not expect any reply\n";
