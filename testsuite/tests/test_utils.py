...
def client_server(client_cmd, client_conf, server_cmd, server_conf):
    ...
    if server_conf:
        server_polyorb_conf = os.path.join(Env().options.testsuite_src_dir,
                                           server_conf)
    ...
    try:
        ...
        if client_conf:
            client_polyorb_conf = os.path.join(Env().options.testsuite_src_dir,
                                               client_conf)
...
def local(cmd, config_file, args=None):
    ...
    if config_file:
        config_file = os.path.join(
            Env().options.testsuite_src_dir, config_file)
