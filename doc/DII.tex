...
\paragraph{}As can be expected, CORBA based on a classical client-server architecture where the client and the server communicate
through the so called CORBA Bus.
\paragraph{}There is thus two types of applications using CORBA : 
\begin{itemize}
\item servers, or service furnishers that provide some useful objects to be used by all otjer CORBA users.
\item clients which place some sort of remote procedure calls on the objects provided by the different servers.
\end{itemize}
Figure \ref{fig:CSModel} on page \pageref{fig:CSModel} shows you a schema of this process.
\begin{figure}
\label{fig:CSModel}
\begin{center}
...
\paragraph{}You can notice on this scema that even inside the client or the server, there are two different levels.
The upper level on each side (Stub for the client and Skeleton for the server) are actually an abstraction level that provides langage
independency and distributed facilities. As a matter of fact, CORBA allows the programmers to write code in his prefered langage
without being bored by the distributed aspect. The user, for example uses Object References as if it were local objects and when he calls
methods on them, these are redirected on the stub that performs all the network work.
\paragraph{}Let's see a little more in detail how all this work.

%%% Object Definition in IDL
\section{Object definition in IDL}
\paragraph{}IDL stands for Interface Definition Langage since objects are called interfaces in CORBA.
This langage provides a way of specifying
objects through their interfaces in order to make that both servers and clients be speaking of the same things.
\paragraph{}IDL uses a kind of modified C syntax. There is no point to detail this syntax here, just refer to \cite{OMG3} if you need further
...
Now let's see how this file is used.

%%% Language Mapping
\section{Mapping to a language}
\paragraph{}We said CORBA is langage independant and everyone can program in its prefered language. At this moment, we only introduced a
new language : IDL. How does it help us ?
\paragraph{}It helps since it gives a common description of the object that will be used in all other langages to generate a language
specific code implementing the object. This operation is called mapping. It is done by IDL to whatever compiler that automatically
generate the Stub and Skeleton we saw before in the language you are interested in. As an example, we show here some result of this
mapping on the calculator object (only the specification is presented here) :
\begin{itemize}
\item In C++ (using omniOrb)
\begin{verbatim}
class calculator : public virtual omniObject, public virtual CORBA::Object {
public:
...
if you are interested. Both writter of the client and server part can then use the mapping they prefer with a very convenient 
...
but what about a distributivity ? Can my client and my server be on different machines, in different places ? Yes of course and this is
provided through the definition of the GIOP : the {\bf G}eneral {\bf I}nter{\bf O}RB {\bf P}rotocol.
...
as if he was only using local objects. Note that this GIOP is also the key of the language independency since the bytes that are sent
...
have the IDL file here. It can seem a bit odd but it can be very usefull. Most of this document is devoted to the explanation of
this feature and its implementation in AdaBroker so you know where to find some information.
...
use some object that didn't even exist when the code was written.
...
know are cosNaming, cosTime and cosEvent (I let you guess what kind of service they provide).
...
The first version (beta 0.8) was released in the summer 99 and already provided most of the existing CORBA fetures.
...
\begin{itemize}
\item {\bf idlac} as an idl to Ada compiler, full written in Ada by the AdaBroker team
\item {\bf broca} as an ORB, also full written in Ada by the AdaBroker team
\end{itemize}
...
\end{verbatim}
\paragraph{}You can notice here the way of handling {\it Any} types or argument lists. For a more complete description of this, please see part \ref{BigPicture} on page \pageref{BigPicture}.
...
manually or to use some generated code (see chapter \ref{UsingGeneratedCode}, page \pageref{UsingGeneratedCode})
...
own pointer that no other object knows and all the pointed values are independant. Figure \ref{fig:ValueMemManag}
on page \pageref{fig:ValueMemManag} gives a 
...
\end{center}
\end{figure}
\paragraph{} In the case of the Any type, you'll have to duplicate the field The\_Value and that's why we provide the following methods :
\begin{verbatim}
...
saying than one more reference on the value was created. Figure \ref{fig:refMemManag} on page \pageref{fig:refMemManag}
...
The only place where it is put to {\it True} is during the creation of a request.
...
\paragraph{} The fact that some values are pointed to by several Any object raises a problem in the case of a multi task environment.
...
\paragraph{}In order to achieve this goal, you have to controll every access to the {\it The\_Value} and {\it Ref\_Counter} field of the
...
On the other side, the {\it Any\_Lock} field of each Any object provides a lock, shared, as the {\it The\_Value} field, 
by all Any pointing to the same value and insuring you that only one of them will access the Any at the same time.
...
a new descendant of {\it CORBA.Impl.Object} containing this list. The list by itself is provided by the {\it Sequences.Unbounded} package.
...
\item{\bf Free, Free\_Memory} : They are not implemented (or at least do nothing). Actually the Object type is sort of controlled and
...
does not exist. However they are needed by the dynamic invocation and thus, some issues were issued concerning this point.
The packages presented here are a kind of translation of the C++ Mapping specification, adapted to Ada.
...
Please report to section section \ref{NVList} of chapter \ref{NV-NVList} page \pageref{NVList} for further details.
...
\ref{NVList} on page \pageref{NVList} and replace {\it NamedValue} by {\it TypeCode} for {\it ExceptionList} or by {\it CORBA.String} for 
{\it ContextList}.
...
index of the first {\it TypeCode} from the list denoting an exception called {\it Name}. That's at least used in the {\it invoke} method
...
detailing its implementation and the different methods provided.
...
package. Here is the Ada their specification :
\begin{verbatim}
   procedure Create_Request
     (Self      : in     Ref;
      Ctx       : in     CORBA.Context.Ref;
...
\begin{itemize}
\item {\bf Ctx} : the context of the invocation. The context object is not described in this documentation since this fonctionality is not
...
\item {\bf Args\_List} : the list of arguments to pass to this operation. You can put a null list here and add some arguments to the request
later if you want (by using {\it Add\_Arg}).
...
\item {\bf Req\_Flags} : some flags dealing with the memory management. As the former : not used.
\end{itemize}
...
\begin{itemize}
\item{\bf Create\_Request} These methods take as parameters all what is needed to make the create the request. The first one just omit
the exception list and the context list and to put to default values. Each argument of these fonction is directly put into the
...
marshalling the request first (object reference, operation name, arguments and perhaps context), then call the ORB to send the
buffer to the server and wait for the response. When the reply comes, it unmarshalls the result (or the exception if any) and puts it
in the {\it Result} field (or raise it). Note that the call is blocking : {\it Invoke} will only return once the result is known.

\item{\bf Delete} deletes a request and frees the associated memory.

\item{\bf Send} This method is the pending of {\it Invoke} for non blocking calls. It only marshall the elements of the request and calls
the ORB but does not wait for any answer. It means that but gives you the control right back without waiting the processing of your
call to finish.
...
\bibitem[OMG3]{OMG3} The Common Object Request Broker:Architecture and Specification Revised Edition: July 1995
