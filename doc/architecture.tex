...
répartis d'\ada{} (\dsa{}, \emph{Distributed Systems Annex}) et
Java/RMI utilisent des variantes distinctes
...
Chacune de ces façades possède plusieurs variantes, correspondant
chacune à un type de plate-forme de répartition (par exemple \dsa{},
CORBA ou Java/RMI)~; le choix d'une variante pour une façade détermine
une personnalité sur cette façade.
...
\ifexamples
  \item[Exemple]
  CORBA définit GIOP (\emph{Generic Inter-ORB Protocol}), un
  protocole permettant l'échange de messages entre intergiciels,
  fournissant les primitives nécessaires au modèle
  «~objets répartis~».
\fi
  \end{description}
...
\item gestion de collections d'entités telles que séquences,
  ensembles non ordonnés, tableaux de taille variable, etc~;
...
\item[Couche applicative] \emph{(en haut)}
  Les modules de l'intergiciel permettant aux objets de l'application
...
Ce service est réalisé en concrétisant un certain nombre de classes
abstraites~:
\begin{description}
...
\backslashbox{Client}{Serveur}
& \RH{\droopi{} no tasking, BOA}
& \RH{\droopi{} tasking, POA}
& \RH{\adabroker{}}      \\ \hline\hline
\droopi{}    & 13,36 & 31,66 & 8,64 \\ \hline
\adabroker{} & 18,23 & 36,27 & 26,20 \\ \hline
\end{tabular}
}
