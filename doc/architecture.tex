...
\item[Binding]\mbox{}
  ...
  \begin{description}
  \item[Definition]
  The binding service consists in associating a transport connection
  (as defined by the following Transport service) with an object
  reference, and to create a client-side surrogate of the actual
  designated object. This surrogate processes method calls performed
  using the object reference by forwarding them as invocation request
  messages to the target object.

  \item[Properties]
  Implicit or explicit binding.

  \item[Rationale]
  Stefani \& Shapiro. Allow flexible control of the transport
  connection establishment, eg QoS.

  %%  XXX éviter les répétitions par rapport à l'autre endroit
  %%     où on cause de ce service.
...
\subsection{Binding} % XXX traduction !

\funcfig{binding}{Fonction \emph{binding}}

\emph{Fonction faisant intervenir seulement la couche protocolaire.
Cf. figure \ref{fig:func-binding}.}

Étant donnée une référence désignant un objet distant, le service
de binding a pour rôle de créer, sur un client, une structure qui
...
%  are associated within a client-side construct. This construct
%  is the result of \emph{binding} the object reference to the
%  transport connection; it can process method invocations (by
%  forwarding them to the actual object through the connection).
%  It can therefore be construed as representing the called object:
%  it is a \emph{surrogate} of the server object on the client side.

%  The binding operation can be implicit or explicit. It is implicit
%  when it is conducted
%  by the middleware as a consequence of the caller performing a
%  method invocation on a yet unbound reference. This is the most
%  common case in distributed object platforms such as CORBA
%  or Java/RMI. It can also be explicit when it is controlled
%  by actions of the user. In the case of an explicit binding of
%  an object reference to a transport object, the user can control
%  parameters of the transport connection, such as the negociation
%  of a level of quality of service.
