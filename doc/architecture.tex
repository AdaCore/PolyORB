...
\begin{figure}
\begin{center}
\mbox{\psfig{file=droopi-design.eps,width=15cm}}
\end{center}
...
\textbf{Contexte~:}
  ...
  ensemble par invocation de méthode selon des motifs
  quelconques (graphe non orienté).

\textbf{Problème~:}
\begin{enumerate}
\item vues croisées~: la spécification d'un composant décrit
  les services offerts aux clients de ce composant, et
  les autres composants avec lesquels il est en relative.

  Pour décrire ces relations, le composant a besoin d'avoir
...
Des messages synchrones (équivalents à des appels de méthoes) peuvent
alors être émis sur le port, et reçues par le composant destinataire.
Lorsqu'un composant doit recevoir un message, sa méthode \emph{Handle\_Message}
est exécutée.
...
Comme un TE, un TSAP a une vue comme source d'événements asynchrone,
