...
\subsubsection{Autres protocols}

Question~: supporte-t-on d'autres protocoles que GIOP (qui peut
éventuellement être instancié sur d'autres moyens de communication
que TCP~: SSL/TLS, ligne série)~? \emph{(protocole GLADE, SunRPC)}

Supporte-t-on un protocole similaire sur un autre support que connecté~?
\emph{(UDP, multicast)}

Si on se limite à la seule famille de protocoles dérivée de GIOP (et
au modèle de types de CORBA, sachant que l'annexe peut s'y réduire,
cf. CIAO)~: la description d'un service en IDL fait office de
description «~de référence~» => simplification du problème des traductions
de services (les mécanismes de traduction doivent être idempotents à
défaut d'être bijectifs).
