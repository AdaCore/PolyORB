      ...
      case Get_Kind (Node.all) is
         ...
         when K_Operation =>
            ...
            if Get_Kind (Operation_Type (Node).all) /= K_Void then
               New_Line (CU);
               Put (CU, "  return ");
               Gen_Node_Stubs_Spec (CU, Operation_Type (Node));
            end if;
         ...
         when K_Param =>
            ...
            Gen_Node_Stubs_Spec (CU, N_Root_Acc (Declarator (Node)));
            case Mode (Node) is
               when Mode_In =>
                  Put (CU, " : in ");
               when Mode_Out =>
            ...
            Gen_Node_Stubs_Spec (CU, Param_Type (Node));
         ...
         when K_Union =>
            ...
            begin
               ...
               while not Is_End (It) loop
                  ...
                  Next (It);
                  Gen_Node_Stubs_Spec (CU, Case_Node);
               end loop;
            end;
