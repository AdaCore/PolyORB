...
package Tree is
   ...
   --  An idl is represented as a tree, which is inspired by the interface
   --  repository (aka IR).
   --  However, contrary to the IR, it doesn't use multiple inheritance.

   type Repository_Id is new String_Cacc;
   type Identifier is new String_Cacc;
   type Version_Spec is new String_Cacc;

   --  A list of node can be built using Node_List.
   --  This is in fact a single linked list, implemented like lisp lists.
   --  The Nil atom is Nil_List.
   type Node_List is private;
   Nil_List : constant Node_List;

   --  Simple way to iterate over a node_list.
   --  NODE_ITERATOR is a type representing an iterator, which must be
   --  initialiazed by INIT.
   --  End of list is detected by IS_END.
   --  Until the end of list is reached, the node can be extracted with
   --  GET_NODE and the iterator can be incremented with NEXT.
